...
const PASSOS_GUIA: PassoGuia[] = [
  {
    ...
    texto: 'Na página inicial, toque em Lista para preparar os produtos antes de ir ao mercado.',
    dica: 'O fluxo principal segue Lista → Compras → Histórico.',
    cena: 'inicio',
  },
  {
    titulo: 'Abra a criação de lista',
    texto: 'Na tela Criar Lista, digite o produto, informe a quantidade e escolha a categoria antes de adicionar o item.',
    dica: 'Você pode montar a lista aos poucos e revisar tudo antes de salvar.',
    cena: 'criar-lista',
  },
  {
    titulo: 'Adicione cada produto',
    texto: 'Conforme você digita, o sistema sugere produtos do catálogo. Você também pode usar um nome personalizado e escolher a categoria desejada.',
    dica: 'As categorias ajudam a manter os itens organizados durante a compra.',
    cena: 'adicionar-itens',
  },
  {
    titulo: 'Salve e dê um nome à lista',
    texto: 'Quando terminar, toque em Salvar Lista. O sistema abre o menu de nomeação acima da página para você identificar a lista.',
    dica: 'Use nomes fáceis de reconhecer, como “Compra do mês”.',
    cena: 'salvar-lista',
  },
  {
    titulo: 'Reabra ou renomeie listas salvas',
    texto: 'Na lateral da tela de Lista ficam as listas que você já salvou. Toque em uma delas para reabrir e continuar editando, ou use o lápis para mudar apenas o nome.',
    dica: 'Assim você pode reaproveitar uma lista sem precisar começar do zero.',
    cena: 'listas-salvas',
  },
  {
    titulo: 'Escolha a lista em Compras',
    texto: 'A lista salva aparece no catálogo de Compras. Toque no card para abrir as opções e então escolha se deseja iniciar ou agendar a compra.',
    dica: 'O próprio card abre as ações; basta tocar ou clicar nele.',
    cena: 'catalogo',
  },
  {
    titulo: 'Registre a compra em andamento',
    texto: 'Durante a compra, marque os itens, informe o preço, ajuste a unidade ou o peso e acompanhe o total parcial conforme avança.',
    dica: 'O check fica à esquerda do item e é marcado automaticamente quando preço e quantidade estão preenchidos.',
    cena: 'compra',
  },
  {
    titulo: 'Consulte ou refaça pelo Histórico',
    texto: 'Depois de finalizar, a compra aparece no Histórico. Toque no card para abrir as ações e então escolha entre refazer a compra ou consultar seus itens.',
    dica: 'Ao refazer uma compra, os preços antigos não são reutilizados.',
    cena: 'historico',
  },
];
...
function IlustracaoGuia({ cena }: { cena: CenaGuia }) {
  if (cena === 'inicio') {
    return <MolduraSite><div className="ajuda-ilustra-home"><img className="ajuda-ilustra-logo" src={logoTitulo} alt="" /><div className="ajuda-home-botao ajuda-destaque">Lista <span>📝</span></div><span className="ajuda-home-seta">↓</span><div className="ajuda-home-botao">Compras <span>🛒</span></div><span className="ajuda-home-seta">↓</span><div className="ajuda-home-botao">Histórico <span>📚</span></div></div></MolduraSite>;
  }

  if (cena === 'criar-lista') {
    return <MolduraSite><div className="ajuda-ilustra-pagina"><div className="ajuda-titulo-lista-real"><img src={iconeLista} alt="" /><strong>Criar Lista</strong></div><div className="ajuda-form-real ajuda-destaque"><label>Produto</label><div className="ajuda-input-real">Digite um produto...</div><div className="ajuda-dupla-real"><div><label>Quantidade</label><div className="ajuda-input-real">1</div></div><div><label>Categoria</label><div className="ajuda-input-real">Mercearia ▾</div></div></div><div className="ajuda-botao-laranja">Adicionar item</div></div></div></MolduraSite>;
  }

  if (cena === 'adicionar-itens') {
    return <MolduraSite><div className="ajuda-ilustra-pagina"><div className="ajuda-titulo-lista-real"><img src={iconeLista} alt="" /><strong>Criar Lista</strong></div><div className="ajuda-form-real ajuda-destaque"><label>Produto</label><div className="ajuda-input-real ajuda-input-ativo">Arroz</div><div className="ajuda-sugestoes-real"><span>Arroz Agulhinha</span><span>Arroz Branco</span><span>Arroz Integral</span></div><div className="ajuda-dupla-real"><div><label>Quantidade</label><div className="ajuda-input-real">2</div></div><div><label>Categoria</label><div className="ajuda-input-real">Mercearia ▾</div></div></div></div></div></MolduraSite>;
  }

  if (cena === 'salvar-lista') {
    return <MolduraSite><div className="ajuda-ilustra-pagina ajuda-cena-animada ajuda-cena-salvar"><div className="ajuda-lista-itens-real"><div><span>Arroz</span><b>2 un.</b></div><div><span>Leite</span><b>6 un.</b></div><div><span>Banana</span><b>1,200 kg</b></div></div><div className="ajuda-botao-laranja ajuda-botao-salvar-demo">Salvar Lista</div><CursorAnimado className="ajuda-cursor-salvar" /><div className="ajuda-mini-overlay-demo" /><div className="ajuda-modal-nome-real"><strong>💾 Nomear sua lista</strong><span>Dê um nome para identificar essa lista.</span><div className="ajuda-input-real">Compra do mês</div><div className="ajuda-botao-laranja">Confirmar e salvar</div><button type="button">Voltar</button></div></div></MolduraSite>;
  }

  if (cena === 'listas-salvas') {
    return <MolduraSite><div className="ajuda-ilustra-pagina ajuda-cena-listas-salvas"><aside className="ajuda-painel-listas-real"><h3>📋 Minhas Listas</h3><div className="ajuda-lista-lateral-demo ajuda-destaque"><div><div className="ajuda-nome-lateral"><strong>Compra do mês</strong><span>✏️</span></div><small>3 itens</small><em>Modificada em 29/08/2026</em></div><b>🗑️</b></div><div className="ajuda-lista-lateral-demo"><div><div className="ajuda-nome-lateral"><strong>Churrasco</strong><span>✏️</span></div><small>8 itens</small><em>Modificada em 27/08/2026</em></div><b>🗑️</b></div></aside><div className="ajuda-lista-edicao-demo"><div className="ajuda-titulo-lista-real"><img src={iconeLista} alt="" /><strong>Criar Lista</strong></div><div className="ajuda-form-real"><label>Produto</label><div className="ajuda-input-real">Digite um produto...</div><div className="ajuda-botao-laranja">Adicionar item</div></div></div><CursorAnimado className="ajuda-cursor-lista" /><CursorAnimado className="ajuda-cursor-lapis" /></div></MolduraSite>;
  }
  ...
  if (cena === 'compra') {
    return <MolduraSite><div className="ajuda-ilustra-pagina ajuda-compra-real"><div className="ajuda-topo-compra-real"><div><img src={iconeLista} alt="" /><strong>Compra do mês</strong></div><span>50% concluída</span><b>Total parcial: R$ 31,69</b></div><div className="ajuda-secao-compra-real"><div className="ajuda-secao-titulo"><strong>Geral (Todos)</strong><span>2 Itens ▲</span></div><div className="ajuda-linha-compra-real ajuda-destaque"><span className="ajuda-check-quadrado marcado">✓</span><span className="ajuda-icone-item">🛒</span><strong>Arroz</strong><div className="ajuda-campo-tracejado">R$ <b>24,90</b></div><div className="ajuda-medida-demo">Un. <i /></div><div className="ajuda-campo-tracejado">2</div><span className="ajuda-total-item">R$ 49,80</span><span className="ajuda-lixo-demo">⌫</span></div><div className="ajuda-linha-compra-real"><span className="ajuda-check-quadrado" /><span className="ajuda-icone-item">🍌</span><strong>Banana</strong><div className="ajuda-campo-tracejado">R$ <b>6,79</b></div><div className="ajuda-medida-demo">Kg. <i className="ativo" /></div><div className="ajuda-campo-tracejado">1,200</div><span className="ajuda-total-item">R$ 8,15</span><span className="ajuda-lixo-demo">⌫</span></div></div></div></MolduraSite>;
  }
      ...
      {guiaAberto && (
        <div className="ajuda-guia-overlay" role="presentation">
          <section
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Guia de uso do Liste & Compre"
          ...
          >
            ...
            <div className="ajuda-guia-corpo" key={`${sessaoGuia}-${passoAtual}`}>
              ...
              <div className="ajuda-guia-texto"><h2>{passo.titulo}</h2><p>{passo.texto}</p><div className="ajuda-guia-dica"><span aria-hidden="true">💡</span><span>{passo.dica}</span></div></div>
            </div>
            ...
            <div className="ajuda-guia-rodape">
              <div className="ajuda-guia-bolinhas" aria-label="Etapas do guia">{PASSOS_GUIA.map((item, indice) => <button key={item.titulo} type="button" className={`ajuda-guia-bolinha${indice === passoAtual ? ' ativa' : ''}`} onClick={() => setPassoAtual(indice)} aria-label={`Ir para o passo ${indice + 1}`} aria-current={indice === passoAtual ? 'step' : undefined} />)}</div>
              <span className="ajuda-guia-teclas">Use A / D ou ← / → para navegar</span>
            </div>
