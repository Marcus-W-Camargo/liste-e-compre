...
export function limparDadosLocaisDaConta(usuarioId: string) {
  ...
  for (const prefixo of prefixos) sessionStorage.removeItem(`${prefixo}${usuarioId}`);
  for (let i = localStorage.length - 1; i >= 0; i -= 1) {
    const key = localStorage.key(i);
    if (key && key.includes(usuarioId) && key.startsWith('liste-e-compre:')) localStorage.removeItem(key);
  }
}
...
export async function excluirConta(usuarioId: string) {
  ...
  const resposta = await fetch('/api/account', { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
  const resultado = await resposta.json().catch(() => ({}));
  if (!resposta.ok) throw new Error(resultado?.error || 'Não foi possível excluir a conta agora.');
